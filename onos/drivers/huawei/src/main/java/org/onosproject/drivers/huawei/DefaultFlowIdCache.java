...
@Service
public class DefaultFlowIdCache implements FlowIdCache {
    private final Map<String, Pair<Integer, Integer>> cache = new HashMap<>();

    @Override
    public Pair<Integer, Integer> get(String flowId) {
        return cache.get(flowId);
    }

    @Override
    public void set(String flowId, int port, int trafficID) {
        cache.put(flowId, Pair.of(port, trafficID));
    }
